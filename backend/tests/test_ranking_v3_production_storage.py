...
def _batch(
    ...
    *,
    ...
    source = RankingV3ProductionBatchInput.create(
        ...
        selections=selections or (_selection(1, prefix=prefix), _selection(2, prefix=prefix)),
    )
...
def _insert_snapshots(engine, selections) -> None:
...
def test_same_session_drift_and_idempotency_key_drift_are_rejected(tmp_path):
    ...
    _insert_snapshots(engine, next_day_same_key.selections)
...
def test_selection_and_alias_must_reference_authoritative_rows(tmp_path):
    ...
    with pytest.raises(DBAPIError, match="must reference its immutable batch"):
        with engine.begin() as connection:
            connection.execute(
                text(
...
def test_selection_must_be_a_canonical_batch_member_and_match_snapshot(tmp_path):
    ...
    with pytest.raises(DBAPIError, match="must reference an opportunity snapshot"):
