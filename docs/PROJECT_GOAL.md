...
2026-09-25 后续切换补录：上述全市场扫描 `full-scan-20260925093257-11955b9b` 已成功结束，进度 `5574/5574`、批次 `28/28`、errors `0`、最终 cards `200`，信号日为预期的 2026-09-24。候选池最初因 V3/V4 签名密钥文件为 root:group `0640`、且存放的是 65 字节十六进制文本而返回 HTTP 500；主任务校验各文件为 64 位十六进制加换行后，将其转换为等值的 32 字节原始密钥，属主 `luozhenkun`、权限 `0600`。随后候选池返回 HTTP 200 的新鲜数据，合格 `69`、展示 `30`；这只证明扫描与候选池可用，不证明自然成交或选股收益。

切换前已停止主调度和独立 paper tick，SQLite preflight 通过；保存 8 张账本表 manifest，并制作在线备份 `qagent-20260925T184616+0800.db`（471298048 字节，immutable `quick_check` 通过，账户 1 行、交易事件 0）。停用服务后，current 从 `998b908` 切换到已暂存的精确提交 `5c958d7519c3ad571c65a5027648ff5145388598`；在 root 审批缺失时调用 root-owned `/home/qagent-boot/bootstrap.sh`，确认服务保持 disabled，随后显式启用服务。首次即时健康检查早于 socket 绑定，稍后重验通过：API health 正常，切换前后账本 manifest 字节相等，仍为唯一会话 `paper-session-50fa0927861b`、检查时交易事件 0。主调度已恢复为间隔 1800 秒，独立 tick 已启用、间隔 600 秒且为 `outside_session`；新的自动化周期当时仍在运行。独立 root 审批已重新建立为 `/home/qagent-boot-approved`（root:root `0600`）。本轮受控切换和运行态对账已验证，但平台启动 hook 仍未配置，未做镜像重启或自然交易时段 tick；不能据此认定重启恢复或成交成功。G9 **仍未完成**，G1/G2 所需 bundles 与 frozen models 仍缺失，原验收条件和交易权限不变。

2026-09-25 新 release 自然周期补录：10:48:50–10:49:39 UTC 的周期已结束，`run_count=4`，并为有界行情修复排入第二个扫描 `full-scan-20260925104851-a1a0246d`；约 10:49 UTC 该扫描仍 running、`0/5574`，不能记为完成。周期记录 `paper_created=9`；同一唯一账户的 DB 中有 9 笔 `free` pending 交易、18 条事件（9 条 created、9 条 execution_updated），尚无成交证据。官方 `/api/paper-trades/session` 汇总为 0，是因为这些交易没有官方认证声明；`/api/paper-trades?reporting_scope=legacy` 返回 9 笔 pending，`/api/paper-trades/account-status?provider=free` 的 `current_model` 也返回 9 笔 pending。上述口径差异不表示账户重复或交易已执行。该周期的 `factor_shadow`、`fuyao_shadow` 为 `not_started`，`forward_evidence` 有 idle 错误，不能称所有阶段健康；平台 hook、镜像重启恢复及交易时段 tick 仍待验，G9 保持未完成。

2026-09-25 约 10:59 UTC 备份与健康快照：云端 current release 为 `5c958d7519c3ad571c65a5027648ff5145388598`；第二次修复扫描 `full-scan-20260925104851-a1a0246d` 仍 running，进度 `1200/5574`、批次 `6/28`、errors `0`。主调度 enabled、idle，`run_count=4`、下次计划 11:19:39 UTC；独立 paper update enabled、`outside_session`，attempts/completed 为 `0/0`。主任务调用现有 `backup_sqlite.sh` 制作 `/home/luozhenkun/qagent/backups/qagent-20260925T185857+0800.db`（471298048 字节、`0600`、脚本 `quick_check` 通过，SHA256 `6031ab7501c2de818b7b7f9f845490d224eb6e13417af35ad031e99662e33265`），并从该备份生成只读账本清单 `/home/luozhenkun/qagent/backups/g9-baseline-20260925T185857-ledger.json`（`0600`，SHA256 `3a77a6f1bdd4e083bb93c79440dcbb2c8851811cdc423cf5e636247f0c19a134`）：account_settings 1、paper_trades 9、events 18、positions 0。`sudo check_linux_unattended_health.py` 为 13 pass、1 fail，overall critical 仅因 scheduler last_error：`factor_shadow` 与 `fuyao_shadow` 均 `not_started`、`forward_evidence` idle；backend/frontend、DB、backup、cron、runit 检查均通过。独立 root 审批现为 root:root `0600`，但 `/etc/my_init.d` 缺失、PID 1 为 `/sbin/launcher sc`；平台启动 hook 与受控重启仍未验证。此快照不证明第二次扫描完成、paper 成交、研究阶段健康或重启恢复，G9 仍未完成。

2026-09-25 约 11:24 UTC 扫描续验：同一次第二轮修复扫描 `full-scan-20260925104851-a1a0246d` 已 succeeded，进度 `5574/5574`、批次 `28/28`、errors `0`。候选池 GET 返回 HTTP 200，`data_health` 为 `paper_candidate_freshness_gate=fresh`、`paper_candidate_expected_signal_date=2026-09-24`、`paper_candidate_signal_date_mismatch=0`，`summary.total_candidates=66`、`shown=30`。`account-status` 仍为唯一 `default` 会话 `paper-session-50fa0927861b`，交易 `total=9`、`pending=9`、`open=0`、`closed=0`。11:19 UTC 自然自动化周期达到 `run_count=5`，扫描阶段为 `already_running` 且指向同一任务 ID，`paper_created=0`。这些证据确认第二轮扫描完成及候选池新鲜，不证明交易时段独立 tick、成交、镜像重启恢复或研究链路全阶段健康；G9 仍未完成。
