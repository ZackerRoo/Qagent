...
最新专项及既有安装升级相关 **64 passed（3.57秒）**、正确虚拟环境全量 **2622 passed、3 warnings（333.92秒）**，Ruff及diff检查通过，终审无P1/P2。锁冲突先留不可变run证据；单次evaluate由Linux主线程SIGALRM硬截止，不启动悬挂SQLite线程；receipt发布与崩溃恢复协议保持。云端预安装发现signals已为服务用户0700但evaluations/runs不存在，v1仅暂存且未装cron，保留失败证据；修订常量改为v2。preview只验证signals并报告计划，execute在cron前安全创建/验证三目录的固定路径、非软链、服务用户uid/gid和0700，失败不装cron，rollback保留研究目录。已实现、已完成上述测试，未 commit、未 push、v2未打包、未安装、未部署。cron仍为默认预览模板，拟工作日11:37 UTC（北京时间19:37）；显式安装后仍需自然触发、失败归档、已有手工signal幂等及首个真实成熟窗口验收。未修改模拟盘、正式Ranking、数据库schema/API、后端服务、冻结模型或交易权限；G2-FQ1/G2选股增益仍未证明。
